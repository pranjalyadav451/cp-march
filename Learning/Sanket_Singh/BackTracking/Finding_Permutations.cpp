...
/** *** GLOBALS *** **/
/**
 * Given a string of all unique elements, find all the unique permutations of the
 * string.
 * **/


void printing_permutation(string &s, int idx, int N) {
	if (idx == N) {
		cout << s << endl;
		return;
	}
	...
	for (int i = idx; i < N; i++) {
		if (us.count(s[i]) == 0) {
			us.insert(s[i]);
			swap(s[i], s[idx]);
			printing_permutation(s, idx + 1, N);
			swap(s[i], s[idx]);
		}
	}
}
...
int main() {
	...
	while (tt--) {
		...
		cin >> s;
		/**---------------***-------------**/
		printing_permutation(s, 0, s.size());
	}
}
