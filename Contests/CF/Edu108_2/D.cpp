...
ll solve(ll A[], ll B[], int n)
{
    ...
    {
        ll current = 0;
        for (int l = c, r = c + 1; l >= 0 and r <= n - 1; l--, r++)
        {
            // puts("Even : ");
            // deb2(l, r);
            current += A[l] * B[r] + B[l] * A[r];
            // deb(current);
            // cout << endl;

            sop = max(sop, prefix[l] + current + prefix[n] - prefix[r + 1]);
        }
        current = 0;
        for (int l = c - 1, r = c + 1; l >= 0 and r <= n - 1; l--, r++)
        {

            current += A[l] * B[r] + B[l] * A[r];
            sop = max(sop, A[c] * B[c] + prefix[l] + current + prefix[n] - prefix[r + 1]);
        }
    }
    return sop;
}
